...
var isExistedFile = exports.isExistedFile = function(p){
...
var isExistedDir = exports.isExistedDir = function(p){
  if (!isNonEmptyString(p)) return false
...
var writeFile = exports.writeFile = function(p, data) {
...
var remove = exports.remove = function(p) {
  if (isExistedFile(p)) {
    fs.unlinkSync(p)

  }else if (isExistedDir(p)) {
...
exports.writeJSON = function(p, data, space) {
  writeFile(p, JSON.stringify(data, null, space || 0))
}

exports.noCacheRequire = function(resolvedPath) {
...
exports.isUrl = function(p) {
  return isNonEmptyString(p) && /^https?:\/\//.test(p)
}


exports.isKeyword = function(p) {
  return isNonEmptyString(p) && p.indexOf('.') === -1 && p.indexOf(path.sep) === -1
}
